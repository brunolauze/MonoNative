...
MonoType* Global::GetType(const char *assemblyName, const char *nameSpace, const char *typeName, int genericCount, MonoType **genericTypes)
{
	MonoClass  *kclass = GetClass(assemblyName, nameSpace, typeName, genericCount, genericTypes);
	if (kclass == NULL) return NULL;
	return mono_class_get_type(kclass);
}
